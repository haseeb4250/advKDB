...
int readQuote(char *csv,int h) {
		...
		struct quoteinfo{
			...
			char sym[9];
			int bidSize,askSize,bidPrice,askPrice;
		}quoteTmp;
		int lineNum = 0;
		char *tmp;
		char *timeTmp;
		...
		while (fgets(line, 1024, stream)){
			...
			if(line!=NULL){
				...
				if(lineNum!=0){
					printf("debug 4\n");
					rowinfo = knk(6,ktj(-KN,castTime(quoteTmp.timeHH,quoteTmp.timeMM,quoteTmp.timeSS)), ks((S) quoteTmp.sym), kj(quoteTmp.bidSize), kj(quoteTmp.askSize), kf(quoteTmp.bidPrice), kf(quoteTmp.askPrice));
